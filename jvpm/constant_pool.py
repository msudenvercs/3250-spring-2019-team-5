...
# operands onto the stack.
        self.load_table = {3: load_int, 8: load_string}
...
the constant shall always be loaded on top of the stack"""
        ...
        tag = constant[0]
        self.load_table[tag](constant, stack)


def load_int(constant, stack):
...
def load_string(constant, stack):
    """loads a string onto the stack"""
# the first 3 bytes are the tag and length; the rest is a utf-8 encoded string.
    decoded = constant[3:].decode("utf-8")
    stack.push_op(decoded)
